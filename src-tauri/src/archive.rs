...
// ── Todo Tracker ─────────────────────────────────────────────────
// 从 archives.next_action 字段解析 Todo 条目，todo_status 表追踪完成状态
...
pub struct TodoItem {
    ...
    pub archive_project: Option<String>,
    pub text: String,
    #[serde(rename = "dueDate")]
    pub due_date: Option<String>,
    pub status: String, // "pending" | "done"
    pub group: String,  // "overdue" | "today" | "this_week" | "later" | "no_date"
    #[serde(rename = "completedAt")]
    pub completed_at: Option<i64>,
    pub source: String, // "archive" | "manual"
    pub priority: Option<String>, // "normal" | "high" | "urgent"
    #[serde(rename = "chainId")]
    pub chain_id: Option<String>,
    #[serde(rename = "parentTodoId")]
...
pub struct TodoQueryResult {
    ...
    /// 日历热力图数据: date -> todo_count
    pub heatmap: HashMap<String, usize>,
}

/// 生成稳定的 todo id: hash(archive_id + todo_text + due_date)
fn generate_todo_id(archive_id: &str, text: &str, due_date: Option<&str>) -> String {
    use std::collections::hash_map::DefaultHasher;
    use std::hash::{Hash, Hasher};
    let mut hasher = DefaultHasher::new();
    format!("{}|{}|{}", archive_id, text, due_date.unwrap_or("")).hash(&mut hasher);
    format!("todo_{:016x}", hasher.finish())
}

/// 解析 next_action 字段为 (text, due_date) 列表
/// 格式: "[YYYY-MM-DD] 待办文本" 或 纯文本，多条用 \n 分隔
fn parse_next_action(raw: &str) -> Vec<(String, Option<String>)> {
    let mut results = Vec::new();
            ...
            if let Some(close) = line.find(']') {
                ...
                if !text.is_empty() {
                    ...
                    if NaiveDate::parse_from_str(date_str, "%Y-%m-%d").is_ok() {
                        results.push((text, Some(date_str.to_string())));
                    } else {
                        results.push((line.to_string(), None));
                    }
                } else {
                    results.push((line.to_string(), None));
                }
...
/// 判定 Todo 的分组
fn classify_todo_group(due_date: Option<&str>) -> String {
    let due = match due_date {
        Some(d) => d,
        None => return "no_date".to_string(),
    };
    let parsed = match NaiveDate::parse_from_str(due, "%Y-%m-%d") {
        Ok(d) => d,
        Err(_) => return "no_date".to_string(),
    };
    ...
    if parsed < today {
        ...
        let end_of_week = today + chrono::Duration::days(days_until_sunday as i64);
...
pub fn query_todos(
    ...
    conn.execute(
        ...
        [],
    ).map_err(|e| format!("Create todo_status table error: {}", e))?;

    // 查询所有有 next_action 的归档（含需求推进时间线上的记录节点）
    // LEFT JOIN demands：节点若归属某需求（a.demand_id），用需求标题作为归属展示，
    // 使 todo 区能直接看出该待办来自哪个需求流程；否则回退到 a.project。
    let mut stmt = conn.prepare(
        "SELECT a.id, a.title, COALESCE(d.title, a.project), a.next_action \
         FROM archives a LEFT JOIN demands d ON d.id = a.demand_id \
         WHERE a.next_action IS NOT NULL AND a.next_action != ''"
    ).map_err(|e| format!("SQL error: {}", e))?;

    let archive_rows: Vec<(String, String, Option<String>, String)> = stmt
        .query_map([], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
    ...
    // 查询已有的 todo 状态
    let mut status_map: HashMap<String, (String, Option<i64>)> = HashMap::new();
    {
        let mut status_stmt = conn.prepare(
            "SELECT id, status, completed_at FROM todo_status"
        ...
        for r in status_rows {
            if let Ok((id, status, completed_at)) = r {
                status_map.insert(id, (status, completed_at));
            }
        }
    }
    ...
    for (archive_id, archive_title, archive_project, next_action) in &archive_rows {
        let parsed = parse_next_action(next_action);
        for (text, due_date) in parsed {
            let todo_id = generate_todo_id(archive_id, &text, due_date.as_deref());
            let (status, completed_at) = status_map
                .get(&todo_id)
                .cloned()
                .unwrap_or(("pending".to_string(), None));

            let group = if status == "done" {
                "done".to_string()
            } else {
                classify_todo_group(due_date.as_deref())
            };

            // 热力图: 统计有 due_date 且 pending 的条目
            if status != "done" {
                if let Some(ref d) = due_date {
                    *heatmap.entry(d.clone()).or_insert(0) += 1;
                }
            }
            ...
            items.push(TodoItem {
                ...
                archive_project: archive_project.clone(),
                text,
                due_date,
                status,
                group,
...

    {
        let mut mt_stmt = conn.prepare(
            "SELECT id, text, due_date, project, priority, status, parent_todo_id, chain_id, completed_at FROM manual_todos"
        ).map_err(|e| format!("Manual todo query error: {}", e))?;
        let mt_rows = mt_stmt.query_map([], |row| {
            Ok((
                row.get::<_, String>(0)?,
                row.get::<_, String>(1)?,
                row.get::<_, Option<String>>(2)?,
        ...
        for r in mt_rows {
            if let Ok((id, text, due_date, project, priority, status, parent_todo_id, chain_id, completed_at)) = r {
                let group = if status == "done" {
                    "done".to_string()
                } else {
                    classify_todo_group(due_date.as_deref())
                ...
                items.push(TodoItem {
                    id,
                    archive_id: "manual".to_string(),
                    archive_title: "手动添加".to_string(),
    ...
    // 注：需求级「下一步」字段已废弃（demands 表不再有 next_todo 列）。
    // 需求流程中的下一步待办现统一由「推进时间线记录节点」承载，
    // 即上方 archives.next_action 的查询已覆盖所有需求节点的下一步内容
    // （archives.demand_id 关联到具体需求）。

    // 排序: 按预定完成时间「从近到远」升序（最近要做的排最前），无日期和已完成排最后
    items.sort_by(|a, b| {
        let a_no_date = a.due_date.is_none();
        let b_no_date = b.due_date.is_none();
        let a_done = a.group == "done";
        let b_done = b.group == "done";
        // done 永远排最后；其次无日期；有日期按升序（从近到远）
        if a_done != b_done { return a_done.cmp(&b_done); }
        if a_no_date != b_no_date { return a_no_date.cmp(&b_no_date); }
        a.due_date.cmp(&b.due_date)
    });

    let overdue_count = items.iter().filter(|i| i.group == "overdue").count();
    let today_count = items.iter().filter(|i| i.group == "today").count();
    let this_week_count = items.iter().filter(|i| i.group == "this_week").count();
    let later_count = items.iter().filter(|i| i.group == "later").count();
    let no_date_count = items.iter().filter(|i| i.group == "no_date").count();
    let done_count = items.iter().filter(|i| i.group == "done").count();
...
pub fn update_todo_status(
    ...
    conn.execute(
        "INSERT INTO todo_status (id, status, completed_at) VALUES (?1, ?2, ?3)
         ON CONFLICT(id) DO UPDATE SET status = excluded.status, completed_at = excluded.completed_at",
        rusqlite::params![todo_id, status, completed_at],
    ).map_err(|e| format!("Update todo status error: {}", e))?;

    // 如果是 manual todo（以 mtodo_ 开头），也更新 manual_todos 表
    if todo_id.starts_with("mtodo_") {
        conn.execute(
            "UPDATE manual_todos SET status = ?2, completed_at = ?3 WHERE id = ?1",
            rusqlite::params![todo_id, status, completed_at],
        ).ok();
    }

    Ok(format!("Todo {} -> {}", todo_id, status))
}

/// AI 分析 Todo 列表，给出建议
#[tauri::command]
pub async fn ai_todo_suggestions(
    state: State<'_, AppState>,
) -> Result<String, String> {
    // 收集 pending todos 信息
    let todo_summary = {
        ...
        let mut stmt = conn.prepare(
            "SELECT id, title, project, next_action FROM archives WHERE next_action IS NOT NULL AND next_action != ''"
        ).map_err(|e| format!("SQL error: {}", e))?;
        ...
        let mut done_ids: Vec<String> = Vec::new();
        {
            let mut st = conn.prepare(
                "SELECT id FROM todo_status WHERE status = 'done'"
            ).map_err(|e| format!("Status error: {}", e))?;
            let r = st.query_map([], |row| row.get::<_, String>(0))
                .map_err(|e| format!("Query error: {}", e))?;
            for item in r {
                if let Ok(id) = item { done_ids.push(id); }
            }
        }

        let today = Local::now().format("%Y-%m-%d").to_string();
        let mut summary_lines = Vec::new();
        summary_lines.push(format!("今天是 {}", today));
        summary_lines.push("以下是用户的待办事项列表：".to_string());

        for (archive_id, title, project, next_action) in &rows {
            let parsed = parse_next_action(next_action);
            for (text, due_date) in parsed {
                let tid = generate_todo_id(archive_id, &text, due_date.as_deref());
                if done_ids.contains(&tid) { continue; }
                let group = classify_todo_group(due_date.as_deref());
                let proj_str = project.as_deref().unwrap_or("无项目");
                let due_str = due_date.as_deref().unwrap_or("无截止日");
                summary_lines.push(format!(
                    "- [{}] 待办: {} | 来源: {} | 项目: {} | 截止: {}",
                    group, text, title, proj_str, due_str
                ));
            }
        }
    ...
    let prompt = format!(
        "你是一个产品经理的 AI 效率助手。请根据以下待办事项列表，给出简洁的建议（不超过 200 字）。\n\
        建议应包含：\n\
        1. 优先级建议：哪些待办最紧急需要优先处理\n\
        2. 风险提醒：哪些即将到期或已逾期\n\
        3. 效率建议：是否有可合并处理的同类任务\n\n\
        {}\n\n\
        请用简洁的中文回答，使用 markdown 格式，直接给出建议，不要重复待办内容。",
        todo_summary
    );

    match call_ollama_generate(&prompt).await {
        Ok(resp) => Ok(resp),
        Err(_) => Ok("**AI 建议暂不可用**\n\n请检查 Ollama 服务是否已启动。建议优先处理已逾期和今日到期的待办事项。".to_string()),
    }
}
...
pub async fn delete_archive(
    ...
    // 删除 todo_status 中的关联状态
    conn.execute(
        "DELETE FROM todo_status WHERE archive_id = ?1",
        rusqlite::params![archive_id],
    )
    .ok(); // 忽略错误（表可能不存在）
...
/// 删除需求看板中的单个推进节点，并彻底清理该节点的所有关联信息：
///  1. archives 主记录
///  2. archive_files 附件关联记录
///  3. 由该节点 next_action 派生的 todo_status 完成状态记录
///     （todo_id 由 archive_id + text + due_date 哈希生成，需重新解析 next_action 才能精确命中）
#[tauri::command]
pub async fn delete_archive_node(
    ...
    // 读取节点的 next_action，同时确认记录存在
    let next_action: Option<String> = conn
        .query_row(
            "SELECT next_action FROM archives WHERE id = ?1",
            rusqlite::params![archive_id],
            |row| row.get(0),
        )
        .map_err(|_| format!("归档节点 {} 不存在", archive_id))?;

    // 1. 删除该节点派生的待办完成状态（todo_id 无 archive_id 外键，只能按哈希 id 精确删除）
    if let Some(ref na) = next_action {
        for (text, due_date) in parse_next_action(na) {
            let todo_id = generate_todo_id(&archive_id, &text, due_date.as_deref());
            let _ = conn.execute(
                "DELETE FROM todo_status WHERE id = ?1",
...
/// 手动添加的 Todo 独立存储在 manual_todos 表
#[tauri::command]
pub fn add_manual_todo(
    state: State<'_, AppState>,
    text: String,
    ...
    conn.execute(
        ...
        [],
    ).map_err(|e| format!("Create manual_todos table error: {}", e))?;

    let now = chrono::Local::now().timestamp();
    let id = format!("mtodo_{}", now);

    // 如果有 parent_todo_id，继承其 chain_id；否则用自身 id 作为 chain_id
    let chain_id = if let Some(ref parent_id) = parent_todo_id {
        let parent_chain: Option<String> = conn.query_row(
            "SELECT chain_id FROM manual_todos WHERE id = ?1",
            rusqlite::params![parent_id],
            |row| row.get(0),
        ).ok();
        parent_chain.unwrap_or_else(|| id.clone())
    } else {
        id.clone()
    };

    conn.execute(
        "INSERT INTO manual_todos (id, text, due_date, project, priority, status, parent_todo_id, chain_id, created_at)
         VALUES (?1, ?2, ?3, ?4, ?5, 'pending', ?6, ?7, ?8)",
        rusqlite::params![id, text, due_date, project, priority.unwrap_or("normal".into()), parent_todo_id, chain_id, now],
    ).map_err(|e| format!("Insert manual todo error: {}", e))?;

    // 如果有 parent，将 parent 标记为 done
    if let Some(ref parent_id) = parent_todo_id {
        conn.execute(
            "UPDATE manual_todos SET status = 'done', completed_at = ?2 WHERE id = ?1",
...
pub fn query_today_todo_count(
    ...
    // 1. 从 archives.next_action 统计今日+逾期 pending
    let mut archive_count = 0usize;
    let mut has_high_priority = false;

    let mut stmt = conn.prepare(
        "SELECT id, next_action, priority FROM archives WHERE next_action IS NOT NULL AND next_action != ''"
    ).map_err(|e| format!("SQL error: {}", e))?;
    ...
    if let Ok(mut st) = conn.prepare("SELECT id FROM todo_status WHERE status = 'done'") {
        if let Ok(r) = st.query_map([], |row| row.get::<_, String>(0)) {
            for item in r { if let Ok(id) = item { done_ids.insert(id); } }
        }
    }

    for row in rows {
        if let Ok((archive_id, next_action, priority)) = row {
            for (text, due_date) in parse_next_action(&next_action) {
                let todo_id = generate_todo_id(&archive_id, &text, due_date.as_deref());
                if done_ids.contains(&todo_id) { continue; }
                if let Some(ref dd) = due_date {
                    if dd <= &today {
                        archive_count += 1;
                        if priority.as_deref() == Some("high") || priority.as_deref() == Some("P0") {
    ...
    // 2. 从 manual_todos 统计今日+逾期 pending
    let manual_count: i64 = conn.query_row(
        "SELECT COUNT(*) FROM manual_todos WHERE status = 'pending' AND due_date IS NOT NULL AND due_date <= ?1",
        rusqlite::params![today],
        |row| row.get(0),
    ).unwrap_or(0);

    let manual_high: i64 = conn.query_row(
        "SELECT COUNT(*) FROM manual_todos WHERE status = 'pending' AND due_date IS NOT NULL AND due_date <= ?1 AND priority IN ('high', 'P0', 'urgent')",
        rusqlite::params![today],
        |row| row.get(0),
    ).unwrap_or(0);

    let total = archive_count + manual_count as usize;
    let high = has_high_priority || manual_high > 0i64;

    Ok(serde_json::json!({
        "count": total,
        "hasHighPriority": high,
    }))
}

/// 删除手动 Todo
#[tauri::command]
pub fn delete_manual_todo(
    state: State<'_, AppState>,
    todo_id: String,
) -> Result<(), String> {
    let conn = state.db.lock().map_err(|e| format!("DB lock error: {}", e))?;
    conn.execute(
        "DELETE FROM manual_todos WHERE id = ?1",
        rusqlite::params![todo_id],
    ).map_err(|e| format!("Delete manual todo error: {}", e))?;
    Ok(())
}

/// 编辑 Todo：
/// - manual 来源：直接更新 manual_todos 表
/// - archive 来源：定位源记录（archives.next_action）中
///   哈希 id 匹配的那一行，替换为新的 text + due_date
#[tauri::command]
pub fn update_todo(
    state: State<'_, AppState>,
    todo_id: String,
    ...
    if todo_id.starts_with("mtodo_") {
        conn.execute(
            "UPDATE manual_todos SET text = ?2, due_date = ?3, project = ?4, priority = ?5 WHERE id = ?1",
            rusqlite::params![todo_id, text, due_date, project, priority.unwrap_or("normal".into())],
        ).map_err(|e| format!("Update manual todo error: {}", e))?;
        return Ok(todo_id);
    }

    // 2. 归档来源
    let archive_rows: Vec<(String, String)> = {
        let mut stmt = conn.prepare(
            "SELECT id, next_action FROM archives WHERE next_action IS NOT NULL AND next_action != ''"
        ).map_err(|e| format!("SQL error: {}", e))?;
        let rows = stmt
    ...
    for (archive_id, next_action) in &archive_rows {
        let parsed = parse_next_action(next_action);
        let mut new_lines = Vec::new();
        let mut matched = false;
        for (t, dd) in parsed {
            if generate_todo_id(archive_id, &t, dd.as_deref()) == todo_id {
                let new_line = match &due_date {
        ...
        if matched {
            let new_na = new_lines.join("\n");
            conn.execute(
                "UPDATE archives SET next_action = ?2 WHERE id = ?1",
                rusqlite::params![archive_id, new_na],
            ).map_err(|e| format!("Update archive next_action error: {}", e))?;
            // 编辑后 todo_id 变化，清理旧状态记录，避免遗留脏数据
            let _ = conn.execute("DELETE FROM todo_status WHERE id = ?1", rusqlite::params![todo_id]);
            return Ok(todo_id);
        }
    }

    // 注：需求级 next_todo 已废弃（demands 表不再有该列）。
    // 需求的下一步待办统一落在推进时间线记录节点上，由上方 archives.next_action 分支处理。

    Err(format!("未找到待办 {}", todo_id))
}

    ...
    if (window.__TAURI_INTERNALS__) {
      try {
        for (const p of previews) {
          ...
          try {
            const newPath = await invoke('sync_virtual_name_to_disk', { id: p.id });
            p.newPath = newPath;
          } catch(e) {
        ...
        const previewNameMap = previews.reduce((acc, p) => ({ ...acc, [p.id]: p.newName }), {});
        const previewPathMap = previews.reduce((acc, p) => ({ ...acc, [p.id]: p.newPath || p.path }), {});
        setFiles(files.map(f => previewNameMap[f.id] ? { ...f, virtualName: previewNameMap[f.id], name: previewNameMap[f.id], path: previewPathMap[f.id], active: false } : f));
      } catch (err) {
